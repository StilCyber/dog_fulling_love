...
// Анимация блоков и текста при скролле

const aniItems = document.querySelectorAll('_anim-items');

if (aniItems.length > 0) {
   window.addEventListener('scroll', animOnScroll);
   function animOnScroll(params) {
      for (let index = 0; index < animItems.lenght; index++) {
         const aniItem = animItems[index];
         const aniItemHeight = aniItem.offsetHeight;
         const animItemOffset = offset(aniItem).top;
         const animStart = 4;
         ...
         if ((pageYOffset > animItemOffset - animItemPoint) && pageYOffset < (animItemOffset + animItemHeight)) {
            animItemOffset.classList.add('_active');
         } else {
